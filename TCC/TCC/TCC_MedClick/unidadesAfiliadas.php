...
$stmt = $pdo->query("SELECT * FROM hospitais ORDER BY nome ASC");
$hospitais = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Buscar cidades únicas sem duplicar
$stmtCidades = $pdo->query("SELECT DISTINCT cidade FROM hospitais ORDER BY cidade ASC");
$cidades = $stmtCidades->fetchAll(PDO::FETCH_COLUMN);

// Buscar estados únicos sem duplicar
$stmtEstados = $pdo->query("SELECT DISTINCT estado FROM hospitais ORDER BY estado ASC");
...
<head>
  ...
  <meta charset="UTF-8">
  <title>Unidades Afiliadas - MedClick</title>
...
<main>
  <div class="page-header">
    <h1 class="page-title">Unidades Afiliadas</h1>
    <p class="page-subtitle">Encontre o hospital mais próximo de você e agende sua consulta com facilidade</p>
  </div>

  <div class="controls-row">
    <div class="search-box">
      <input type="text" id="search" placeholder="Buscar hospital por nome, endereço ou especialidade...">
    ...
    <div class="filter-controls">
      ...
      <select class="filter-select" id="filter-state">
        ...
        <?php foreach ($estados as $estado): ?>
          <option value="<?= strtolower(str_replace(' ', '-', trim($estado))) ?>">
  ...
  <div class="results-info" id="results-info">
    Mostrando <span id="results-count"><?php echo count($hospitais); ?></span> de <?php echo count($hospitais); ?> unidades
  </div>
  ...
  <div id="cards-container" class="cards-grid">
    <?php if (!empty($hospitais)): ?>
      <?php foreach ($hospitais as $hospital): ?>
        <?php
          // Caminho da imagem
          $img = !empty($hospital['imagem']) 
              ? "img/hospitais/" . $hospital['imagem'] 
              : "img/default_hospital.jpg";
        ?>
        <div class="card" 
             data-city="<?= strtolower(str_replace(' ', '-', $hospital['cidade'])) ?>" 
             data-state="<?= strtolower(str_replace(' ', '-', $hospital['estado'])) ?>">
             
          <img src="<?= htmlspecialchars($img) ?>" alt="<?= htmlspecialchars($hospital['nome']) ?>">
          
          <div class="card-content">
            <h3 class="card-title"><?= htmlspecialchars($hospital['nome']) ?></h3>
            <p class="card-address">
              <?= htmlspecialchars($hospital['endereco']) ?> - 
              <?= htmlspecialchars($hospital['cidade']) ?> - 
              <?= htmlspecialchars($hospital['estado']) ?>
            </p>
            
            <div class="card-details">
              <?php if (!empty($hospital['telefone'])): ?>
                <span class="detail-item">
                  <i class="fas fa-phone"></i> <?= htmlspecialchars($hospital['telefone']) ?>
                </span>
              <?php endif; ?>
              
              <span class="detail-item">
                <i class="fas fa-hospital"></i> Hospital 24h
              </span>
              
              <?php if (!empty($hospital['email'])): ?>
                <span class="detail-item">
                  <i class="fas fa-envelope"></i> <?= htmlspecialchars($hospital['email']) ?>
                </span>
              <?php endif; ?>
            </div>
            
            <div class="card-badge badge-24h">24 horas</div>
            
            <div class="card-actions">
              <a href="https://www.google.com/maps?q=<?= urlencode($hospital['endereco'].' '.$hospital['cidade'].' '.$hospital['estado']) ?>" 
                 target="_blank" class="action-btn btn-primary">
                <i class="fas fa-map-marker-alt"></i> Como chegar
              </a>
              <?php if (!empty($hospital['telefone'])): ?>
                <a href="https://wa.me/55<?= preg_replace('/\D/', '', $hospital['telefone']) ?>" 
    ...
    <?php else: ?>
      <div class="no-results">
        ...
        <h3>Nenhum hospital cadastrado</h3>
...
function filtrarHospitais() {
  ...
  cards.forEach(card => {
    ...
    const cardEstado = card.getAttribute('data-state');
    
    const termoMatch = nome.includes(termo) || endereco.includes(termo);
    const cidadeMatch = !cidade || cardCidade === cidade;
    const estadoMatch = !estado || cardEstado === estado;
    
    const exibir = termoMatch && cidadeMatch && estadoMatch;
...
document.getElementById('search').addEventListener('input', filtrarHospitais);
document.getElementById('filter-city').addEventListener('change', filtrarHospitais);
document.getElementById('filter-state').addEventListener('change', filtrarHospitais);
